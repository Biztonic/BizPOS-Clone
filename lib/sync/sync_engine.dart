...
class SyncService with ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    _outboundManager = OutboundManager(
      ...
      // Event Listeners for Modular Integration
      EventBus.instance.on<OrderCreatedEvent>((event) {
        try {
          final orderModel = event.order as OrderModel;
          final businessEvent = event.event as BusinessEvent;
          final movements = event.movements as List<dynamic>;

          final hiveSafeData = orderModel.toHiveMap();
          final now = orderModel.date;
          hiveSafeData['businessDayId'] = "${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}_${event.storeId}";
          ...
          queueOperation(
            ...
            docId: businessEvent.id,
            action: 'create',
            payload: businessEvent.toMap(),
          );

          for (var movement in movements) {
            final mv = movement as InventoryMovement;
            queueOperation(
              ...
              payload: {
                ...mv.toMap(),
                'createdAt': mv.createdAt.toIso8601String(),
              },
            );
          }
        } catch (e) {
          debugPrint('❌ SyncService: Error processing OrderCreatedEvent: $e');
        }
      });
  ...
  Future<void> processSync({bool forceManual = false, bool forceFull = false}) async {
    ...
    try {
      ...
      await _outboundManager.processQueue(forceManual: forceManual);
